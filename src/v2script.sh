...
set_proxy() {
  ...
  wget -q https://raw.githubusercontent.com/fwcde/v2ray-tcp-tls-web/${branch}/config/tls-shunt-proxy.yaml -O /tmp/config_new.yaml
...
get_caddy() {
  if [ ! -f "/usr/local/bin/caddy" ]; then
    ...
    curl -sL https://raw.githubusercontent.com/fwcde/v2ray-tcp-tls-web/${branch}/tools/getcaddy.sh | ${sudoCmd} bash -s personal
    ...
    wget -q https://raw.githubusercontent.com/fwcde/v2ray-tcp-tls-web/${branch}/config/caddy.service -O /tmp/caddy.service
...
build_web() {
  if [ ! -f "/var/www/html/index.html" ]; then
    ...
    local template="$(curl -s https://raw.githubusercontent.com/fwcde/web-templates/master/list.txt | shuf -n  1)"
    wget -q https://raw.githubusercontent.com/fwcde/web-templates/master/${template} -O /tmp/template.zip
    ${sudoCmd} mkdir -p /var/www/html
    ${sudoCmd} unzip -q /tmp/template.zip -d /var/www/html
    ${sudoCmd} wget -q https://raw.githubusercontent.com/fwcde/v2ray-tcp-tls-web/${branch}/custom/robots.txt -O /var/www/html/robots.txt
  else
...
checkIP() {
  local realIP="$(curl -s `curl -s https://raw.githubusercontent.com/fwcde/v2ray-tcp-tls-web/master/custom/ip_api`)"
...
install_v2ray() {
  ...
  if [[ $(read_json /usr/local/etc/v2ray/config.json '.inbounds[0].streamSettings.network') != "domainsocket" ]]; then
    ...
    wget -q https://raw.githubusercontent.com/fwcde/v2ray-tcp-tls-web/${branch}/config/v2ray.json -O /tmp/v2ray.json
  ...
  local V2_DOMAIN="$(read_json /usr/local/etc/v2script/config.json '.v2ray.tlsHeader')"
  local uuid_tcp="$(read_json /usr/local/etc/v2ray/config.json '.inbounds[0].settings.clients[0].id')"
  local json_tcp="{\"add\":\"${V2_DOMAIN}\",\"aid\":\"0\",\"host\":\"\",\"id\":\"${uuid_tcp}\",\"net\":\"tcp\",\"path\":\"\",\"port\":\"443\",\"ps\":\"${V2_DOMAIN}\",\"tls\":\"tls\",\"type\":\"none\",\"v\":\"2\"}"
  local uri_tcp="$(printf %s "${json_tcp}" | base64 --wrap=0)"
...
install_trojan() {
  ...
  if [ ! -f "/etc/trojan-go/config.json" ]; then
    ...
    wget -q https://raw.githubusercontent.com/fwcde/v2ray-tcp-tls-web/${branch}/config/trojan-go_plain.json -O /tmp/trojan-go.json
...
rm_v2script() {
  ...
  curl -sL https://raw.githubusercontent.com/fwcde/v2ray-tcp-tls-web/${branch}/tools/rm_v2script.sh | bash
  exit 0
}
...
install_mtproto() {
  if [[ $(read_json /usr/local/etc/v2script/config.json '.mtproto.installed') != "true" ]]; then
    ...
    local FAKE_TLS_HEADER="$(curl -s https://raw.githubusercontent.com/fwcde/my-scripts/master/text/mainland_cdn.txt | shuf -n 1)"
...
vps_tools() {
  ...
  wget -q https://raw.githubusercontent.com/fwcde/v2ray-tcp-tls-web/${branch}/tools/vps_tools.sh -O /tmp/vps_tools.sh && chmod +x /tmp/vps_tools.sh && ${sudoCmd} /tmp/vps_tools.sh
  exit 0
}
